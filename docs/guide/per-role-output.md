...
Add `{.@name}` to any element. Untagged content is always included; tagged content only appears when generating for a matching role.
...
Now `resumx resume.md --role fullstack` includes all `{.@frontend}` content, all `{.@backend}` content, and any explicitly tagged `{.@fullstack}` content, along with untagged common content.
